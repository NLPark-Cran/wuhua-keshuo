...
- 🌓 **明暗主题**：支持剧院暗色模式。
- 🎭 **示例剧目**：已内置温州鼓词《白蛇传•水漫金山》选段的真实 AI 解析结果。
...
注意：当前生产部署使用 `next start` 而非 `output: standalone`，以避免 Next.js 16 + Turbopack 下 CSS chunk 丢失的问题。
...
- 转写解析器已针对重复片段优化，但模型仍可能产生少量重复或近似句。

## 📚 开发者文档

更详细的开发规范、环境变量、故障排查请参阅项目根目录 `AGENTS.md`。
